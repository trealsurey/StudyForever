...
- **伪类：** 用于向某些已经存在的选择器添加特殊效果（当状态改变时），以 `:` 开头
- **伪元素：** 用于将特殊效果添加到不存在的虚拟元素中（浏览器自动创建），只在外部显示可见，但不会在文档的源码中找到它们，以 `::` 开头

> `::before` 和 `::after` 元素必须设置 content 属性，否则就不会显示出来，如果是放一张图片就可以设置 `content: '';`

[伪类应用案例：气泡图](../case/%E6%B0%94%E6%B3%A1%E5%9B%BE.html)