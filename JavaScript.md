...
> 客户端发请求 -> 服务端接受请求并返回相应的 HTML 文档 -> 页面刷新，客户端加载新的 HTML 文档

缺点：
- 用户点击页面中的某个按钮向服务器发送请求时，页面本质上只是一些数据发生了变化，而此时服务器却要将重绘的整个页面再返回给浏览器加载，非常浪费网络资源，也有悖于程序开发的 “DRY（Don't repeat yourself）原则

![服务端渲染示意图](imgs/%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%AB%AF%E6%B8%B2%E6%9F%93%E7%A4%BA%E6%84%8F%E5%9B%BE.png)

想要在页面数据变动时，只向服务器请求新的数据，并且在阻止页面刷新的情况下动态替换页面中展示的数据的话，就需要用到 **AJAX**，它可以在不重新刷新页面的情况下与服务器通信，交换数据或者更新页面

使用 AJAX 主要完成以下事情：
- 在不重新加载页面的情况下发送请求给服务器
- 接受并使用从服务器发来的数据

![客户端渲染示意图](imgs/%E5%AE%A2%E6%88%B7%E7%AB%AF%E6%B8%B2%E6%9F%93%E7%A4%BA%E6%84%8F%E5%9B%BE.png)

网页中的资源通常被放在资源服务器中，由浏览器自动发送 HTTP 请求来获取、解析、展示

页面中动态展示的数据，比如：数据展示、搜索结果、表单验证等都是通过在 JS 中发送 HTTP 请求来获取的

![网页中资源的获取示意图](imgs/%E7%BD%91%E9%A1%B5%E4%B8%AD%E8%B5%84%E6%BA%90%E7%9A%84%E8%8E%B7%E5%8F%96%E7%A4%BA%E6%84%8F%E5%9B%BE.png)
...
- 返回值是一个 Promise，可以直接调用 resolve 和 reject
- 不需要像 XHR 一样，所有的操作都在同一个 xhr 对象上

`fetch(input [, init])`
- input：定义要获取的资源地址，可以使一个 URL 字符串，也可以使用一个 Request 对象（试验性特性）
- init：其他初始化参数
  - method：GET/POST...
  - headers：请求头
  - body：请求体
...
> https://anuradha.hashnode.dev/debouncing-vs-throttling

### 防抖

简单理解防抖的含义：
- 事件触发时，相应的函数并不会立即触发，而是会等待一定的时间
- 当事件密集触发时，函数的触发会被频繁的推迟
- 只有等待了一段时间也没有事件触发，才会真正地执行响应函数

![防抖示意图](imgs/debounce-timeline.png)
...
[详细代码](面试题/%E6%89%8B%E5%86%99%E4%BB%A3%E7%A0%81/%E9%98%B2%E6%8A%96%E5%87%BD%E6%95%B0/)

### 节流

简单理解节流：
- 当事件触发时，会 **立即执行** 这个事件的响应函数
- 如果这个事件呗频繁触发，那么节流函数会按照一定的频率来执行函数
- 不管在这个中间有多少次触发这个事件，执行函数的频率总是固定的

![节流示意图](imgs/throttle-timeline.png)

应用场景：
- 和防抖类似
- 某些游戏中，比如飞机大战中按下空格发射子弹，但是即使频率非常快速的按下，子弹也只会保持一定的频率来发射
- ……
...
// JS 默认没有深拷贝
// 3.1 JSON 方法，缺点是不会拷贝函数、Symbol 等内容；如果对象存在循环引用,也会报错
// 3.2 自定义深拷贝函数
const obj4 = JSON.parse(JSON.stringify(info))
// 3.3 如果是函数类型，一般是不需要进行深拷贝的
//     因为函数是用来执行的，对函数进行深拷贝是很浪费性能的
```
